...
export function DeviceSearchModal({ onClose, onPaired }: { onClose: () => void; onPaired: () => void }) {
  ...
  const connect = async (d: DiscoveredDevice) => {
    ...
    setError(null)
    setStatus(t('Confirm the code %1$s on "%2$s", then approve there. Waiting...', d.code, d.name))
    try {
      const r = await pairDevice(d)
      if (r.ok) {
        setStatus(t('Connected to "%s".', r.name ?? d.name))
        onPaired()
        setTimeout(onClose, 700)
      } else {
        setError(r.error ?? t('Pairing failed'))
        setStatus(null)
        setPairing(null)
      }
    } catch (e) {
  ...
  const [noDevicesBefore, noDevicesAfter] = tsplit('No devices found. On your other Mac run %s on the same Wi-Fi.')
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
          <h2 className="text-sm font-semibold text-foreground">{t('Search local devices')}</h2>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="rounded-md px-2 py-1 text-tertiary-foreground hover:text-foreground" aria-label={t('Close')}>
              ✕
            </button>
          </div>
        </div>
        ...
        <div className="px-5 py-4">
          {scanning ? (
            ...
            <p className="py-6 text-center text-sm text-tertiary-foreground">
              {noDevicesBefore}
              <span className="font-mono text-foreground">codeburn share</span>
              {noDevicesAfter}
            </p>
          ) : (
            <div className="flex flex-col gap-2">
              {found.map((d) => (
                <div key={d.fingerprint} className="flex items-center gap-3 rounded-md border border-border px-3.5 py-3">
                  ...
                  {d.paired ? (
                    <span className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">{t('Connected')}</span>
                  ) : pairing === d.fingerprint ? (
                    <span className="font-mono text-xs text-tertiary-foreground">{t('code %s', d.code)}</span>
                  ) : (
                    <button
