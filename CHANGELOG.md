...
- **Preserved source-dead day-slices are repriced through the current LiteLLM tables on the next DAILY_CACHE_VERSION bump.** A carried day-slice - one the partial-survival guard from #1129 kept whole because a fresh parse lost calls - holds per-model token counts, so the post-bump re-derive now recomputes each preserved model's cost from those stored tokens at the current rates. Daily cache v30 mints a fresh filename; the migration writes the carried days with the new flag, the re-derive that fires on it sweeps them, and the entitlement is spent. On a real 200-day corpus three days were under-claimed by ~$46 in aggregate (2026-07-19 was the worst at $15.29/283 calls vs the live-parse $28.86/143 calls for the surviving subset) - those move toward the live numbers, with the rest of the cache byte-identical. The change is honest in both directions: a price cut LOWERS a carried slice, a price raise RAISES it, a model added since the day was derived prices the slice for the first time. The never-lose rule is about data PRESENCE, not direction - a model absent from the current tables keeps its stored cost (the calculateCost lookup is gated on getModelCosts so a missing model can never silently zero a priced slice), and a slice without per-model token detail (a pre-v15 cache, or a modern provider whose parser never emitted per-model splits) keeps its stored cost untouched. Calls, sessions, tokens, savings, edit/one-shot turns, categories and projects are never touched - only the derived cost and the day-level rollup that sums it move. The re-derive's guarded merges (#1129 completeness gates, partial-survival) are unchanged: slices whose sources still exist already reprice via the normal re-parse, and the carry-forward path is the only one this touches. The re-derive fires once and never on ordinary launches, and a partial re-parse holds the entitlement until a complete one lands. (#1132)
