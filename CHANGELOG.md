...
- **A first `q` during the cold-start background fill arms quit and shows a status line, instead of holding the terminal for ~16.5s with no feedback.** The progressive cold start drains the deferred history in the background after first paint, and pressing `q` while it ran blocked on the drain's cache save before exiting - a deterministic silent wait on a 21k-file corpus (#1143). The dashboard's `useInput` now treats the first `q` during an active fill as "let the drain finish": it stays mounted, the fill continues, and a `quitting Finishing background index so the next launch starts warm - press q or Ctrl+C again to quit now` line lands in the dashboard footer (matching the `indexing` banner's color and shape). A second `q`, or any `Ctrl+C` at any time during the drain, exits through the same abrupt path the rest of the app uses - #1109 made that path kill-safe, so resume still converges and nothing is marked seen without being parsed. With no fill in flight, `q` still exits immediately and the status line never appears. Pinning test: `tests/dashboard-quit-fill.test.ts` covers all four transitions (first q arms + renders, second q exits, Ctrl+C during the drain exits, q with no fill exits without the line).
