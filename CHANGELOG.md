...
- **Preserved source-dead day-slices are repriced through current pricing tables from their stored per-model tokens.** The partial-survival guard correctly refuses to replace a carried day-slice when the fresh parse has fewer calls - the source files have aged out - but that pins the slice to its HISTORICAL prices too. After a repricing wave (#1118 orcarouter, #1123 copilot, #1101, ...) days whose sources only PARTLY survived under-claimed: measured 3 days across a 200-day corpus, ~$46 total. Preserved v14+ slices carry per-model token counts, so a daily-cache v30 bump re-derives their COST from current tables once and stamps `preservedRepriced`; ordinary launches and savings-hash / tz re-derives leave the stamp alone and skip the pass. The reprice follows the tables honestly in BOTH directions: a price cut lowers a slice, a price hike raises it. The never-lose rules are about data presence, not direction: a model absent from current tables keeps its stored cost (never zero a priced slice), a slice without per-model token detail (pre-v15 era or a provider without per-model splits) keeps its stored cost entirely, and providers whose stored cost is RECORDED rather than token-derived (hermes `actual_cost_usd` / `estimated_cost_usd`, cline-cli `costReported`, vercel-gateway `total_cost`, openclaw `costFromProvider`, devin `committedAcuCost`, kiro credits) are skipped wholesale - re-pricing their tokens would replace a recorded cost with a token estimate and lose truth rather than fix it. Calls, tokens, sessions, savingsUSD, categories and projects are never touched - only the derived cost moves, and only for slices whose `models` map actually exists. Surviving-source slices already reprice through the normal re-parse path and are unaffected here. (#1132)
- **Repricing excludes providers whose recorded cost is not token-derived.** The `NON_TOKEN_PRICED_PROVIDERS` set (hermes, cline-cli, vercel-gateway, openclaw, devin, kiro) is skipped wholesale by the reprice pass: a recorded `cost` on those slices is the source of truth, and re-deriving it from per-model tokens would clobber a real number with an estimate.
